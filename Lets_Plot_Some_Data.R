...
# Using HEX colour codes in a manual scale:

p1 + coord_polar() + scale_fill_manual(values = c('#ccece6', '#99d8c9', '#66c2a4', '#2ca25f','#006d2c'))

# Creating a second plot to practise using HEX colour codes in a manual scale:

p2 <- ggplot(aes(x = carat), data = diamonds)
p2 + geom_histogram(aes(fill = cut)) + scale_fill_manual(values = c('#a6611a', '#dfc27d', '#f5f5f5', '#80cdc1', '#018571')) + theme(panel.background = element_rect(fill = 'black'))
